...
const String _baseUrl = BdappsConfig.baseUrl; // https://www.bdappsdigitalapps.com/NADB26105_FinalProject/
...
/// Wrapper for GoRouter and backwards compatibility
...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _onContinue() async {
    ...
    if (!_isSupportedRobiAirtelNumber(phone)) {
      _showError('সঠিক Robi/Cirkle নম্বর দিন (০১৮ অথবা ০১৬)');
      return;
    }
    ...
    try {
      ...
      if (success && referenceNo.isNotEmpty) {
        ...
        String errorMsg;
        if (message.contains('503') || message.contains('HTML instead of JSON')) {
          errorMsg = 'bdapps গেটওয়ে সার্ভারে সাময়িক সমস্যা হচ্ছে (HTTP 503 Service Unavailable)। কিছুক্ষণ পর আবার চেষ্টা করুন অথবা উপরে দেওয়া SMS / USSD দিয়ে সাবস্ক্রাইব করুন।';
        } else if (message.isNotEmpty) {
          errorMsg = message;
        } else if (statusDetail.isNotEmpty) {
          errorMsg = statusDetail;
        } else {
          errorMsg = 'OTP পাঠানো যায়নি। আবার চেষ্টা করুন।';
        }
        _showError(errorMsg);
      }
    } catch (e) {
  ...
  Future<void> _saveAndGoHome(String phone) async {
    ...
    if (widget.authService != null) {
      await widget.authService!.onLoginSuccess(phone);
    }
    if (!mounted) return;
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      Text(
                        'SMS: START 11217 to 21213 | USSD: *21213*10757#',
                        textAlign: TextAlign.center,
                        style: TextStyle(
...
class _OtpVerifyPageState extends State<OtpVerifyPage> {
  ...
  Future<void> _verifyOtp() async {
    ...
    try {
      ...
      if (statusCode == 'S1000' || successFlag) {
        ...
        // INITIAL CHARGING PENDING both grant app access; everything else blocks it.
        ...
        if (widget.authService != null) {
          await widget.authService!.onLoginSuccess(widget.phone);
        }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('যাচাইকরণ'), centerTitle: true),
      body: SafeArea(
        child: SingleChildScrollView(
          child: Padding(
